...
export default { 
    // components: { [Tag.name]: Tag, [Col.name]: Col, [Icon.name]: Icon, [Cell.name]: Cell, [CellGroup.name]: CellGroup, [Swipe.name]: Swipe, [SwipeItem.name]: SwipeItem, [GoodsAction.name]: GoodsAction, [GoodsActionIcon.name]: GoodsActionIcon, [GoodsActionButton.name]: GoodsActionButton }, 
    data() { return { 
        goods: { title: "美国伽力果（约680g/3个）", 
            price: 2680, express: "免运费", remain: 19, thumb: [], }, 
        id: this.$route.params.id , 
        show: false, 
        sku: { tree: [ { k: "选择", // skuKeyName：规格类目名称 
                    v: [ { id: "1215", 
                    name: "手机", 
                    imgUrl: "http://demo.dtcms.net/upload/201504/20/thumb_201504200046594439.jpg" } ], 
    ...
    methods: { 
        ...
        getLunbotu() {
             this.$http.get("api/getthumimages/" + this.id).then(result => { if (result.body.status === 0) { this.goods.thumb = result.body.message; } }); 
        } 
    }
}
