<template>
    <div :key="shuaxin">
       <van-panel v-for="(list,indexList) in lists" :key="indexList" :title="`订单号:${list.orderId}`" status="待付款">
            <!-- 商品卡片 -->
            <div class="commodity">
...
export default {
    ...
    methods:{
        pay(indexList){
            ...
            // this.$store.commit("myOrder/pay",chooseOrder)
    ...
    computed: {
        ...mapState({
            lists:state=>state.lists.filter(item=>{ return item.type == 1 })
        }),
